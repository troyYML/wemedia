...
const splitSlashN = (value?: string): string[] => {
  if (!value) return []
  return value
    .split('/n')
    .map(item => item.trim())
...
const ResultsPage: React.FC = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'hitKeywords',
      width: 220,
      render: (hitKeywords: string) => {
        const keywords = splitSlashN(hitKeywords)
    ...
    },
    {
      ...
      key: 'keywordScore',
      width: 70,
      sorter: (a: Article, b: Article) => (a.keywordScore || 0) - (b.keywordScore || 0),
      render: (score: number) => (
        score > 0 ? <Tag color="green">{score}</Tag> : <span style={{ color: '#666' }}>0</span>
      ),
    },
  ]
