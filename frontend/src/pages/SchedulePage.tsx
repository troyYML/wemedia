...
const SchedulePage: React.FC = () => {
  ...
  const handleOpenModal = (task?: models.ScheduledTask) => {
    if (task) {
      ...
      let recentDays = 30
      let maxPages = 10
      let requestIntervalMin = 2
      let requestIntervalMax = 5
      ...
      try {
        ...
        requestIntervalMin = cfg.requestIntervalMin || cfg.requestInterval || 2
        requestIntervalMax = cfg.requestIntervalMax || cfg.requestInterval || 5
  ...
  const handleSubmit = async () => {
    try {
      ...
      const scrapeConfig = JSON.stringify({
        ...
        requestInterval: values.requestIntervalMin || 2,
        requestIntervalMin: values.requestIntervalMin || 2,
        requestIntervalMax: values.requestIntervalMax || values.requestIntervalMin || 5,
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden', padding: '12px 24px' }}>
      ...
      >
        <Form form={form} layout="vertical" initialValues={{
          ...
          weekday: 1, intervalHours: 6, recentDays: 30, maxPages: 10, requestIntervalMin: 2, requestIntervalMax: 5, includeContent: false,
        }}>
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            ...
            <Form.Item label="间隔下限" name="requestIntervalMin" style={{ flex: 1 }}>
              <InputNumber min={1} max={60} style={{ width: '100%' }} addonAfter="秒" />
            </Form.Item>
            <Form.Item label="间隔上限" name="requestIntervalMax" style={{ flex: 1 }}>
              <InputNumber min={1} max={120} style={{ width: '100%' }} addonAfter="秒" />
            </Form.Item>
          </div>
