		...
		for i := 0; i < len(articles); i += batchSize {
			...
			if err := tx.Clauses(clause.OnConflict{
				...
				DoUpdates: clause.AssignmentColumns([]string{
					"title", "link", "digest", "content", "clean_content", "image_links", "hit_keywords", "keyword_score",
...
func (r *ArticleRepositoryImpl) Search(keyword string, limit, offset int) ([]*models.Article, error) {
	...
	query := r.db.Where("title LIKE ? OR digest LIKE ? OR content LIKE ? OR clean_content LIKE ?",
