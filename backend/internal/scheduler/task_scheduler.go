...
func (ts *TaskScheduler) executeScrape(ctx context.Context, task *models.ScheduledTask) ([]appmodels.Article, error) {
	...
	// 创建爬虫
	minInterval := scrapeConfig.RequestIntervalMin
	maxInterval := scrapeConfig.RequestIntervalMax
	if minInterval <= 0 && maxInterval <= 0 && scrapeConfig.RequestInterval > 0 {
		minInterval = scrapeConfig.RequestInterval
		maxInterval = scrapeConfig.RequestInterval
	}
	if minInterval <= 0 {
		minInterval = 2
	}
...
func (ts *TaskScheduler) saveArticles(articles []appmodels.Article) error {
	...
	for i := range articles {
		...
		dbArticle := &models.Article{
			...
			ImageLinks:       article.ImageLinks,
			HitKeywords:      article.HitKeywords,
			KeywordScore:     article.KeywordScore,
